...
func RegisterRoutes(mux *http.ServeMux, db *gorm.DB, authenticate func(http.Handler) http.Handler) {
	...
	mux.Handle("GET /agreements", authenticate(http.HandlerFunc(controller.ListAgreements)))
	...
	mux.Handle("POST /agreements/{id}/demo-confirm", authenticate(http.HandlerFunc(controller.DemoConfirmAgreement)))
